...
if(@vendedor);
    log( );
    log(&a&l[Macro]&9 &l&k! &9VENDEDOR &l&k! &fparou no armazem &e%#narmazem% &fe bau &e%#nbau% &4&l[OFF]);
    echo(/home mob);
    unset(@vendedor);
    stop();
else;
    set(@vendedor);
    log( );
    log(&a&l[Macro]&9 &l&k! &9VENDEDOR &l&k! &3&l[ON]);
    prompt(#nitens,$$?,Quantos itens diferentes deseja vender?);
    // N° DO ARMAZÉM //
    prompt(&tipo,$$?,Visitar o "mkb" ou "armazem"?);
    if(%&tipo% != "mkb");
        prompt(#narmazem,$$?,Qual sera o primeiro armazem a vasculhar?);
    endif;
    // ITENS A VENDER //
    #nloop = 1;
    do;
    ...
    prompt(#nbau,$$?,Qual o primeiro bau do armazem?);
    prompt(#nbaumax,$$?,Ate qual bau do armazem devo vender?);
    if(%&tipo% != "mkb");
        prompt(#narmazemfinal,$$?,Qual o ultimo armazem que devo vender?);
    else;
        #narmazem = 1;
    ...
    do;
        ...
        do;
            ...
            if((%#slotvazio% > 0) || (%#possuiitens% == 0));
                log(&a&l[Macro]&9 &l&k! &9VENDEDOR &l&k! &cbau %#nbau% vazio!);
                #slotvazio = 1;
                inc(#nbau,1);
                if(%#multbau% == 0);
                    inc(#filabau,1);
            ...
            if(%#nbau% > %#nbaumax%);
                ...
                #nbau = %#nbauoriginal%;
                log( );
                log(&a&l[Macro]&9 &l&k! &9VENDEDOR &l&k! &cArmazem %#narmazem% limpo!);
                break;
            endif;
        while(%#slotvazio% > 0);
        #nloop = 1;
        do;
            log(&a&l[Macro]&9 &5Inventario com produtos. &l&k! &9VENDEDOR &l&k! &fpassando item &e%#item[%#nloop%]%);
        ...
        if(%#narmazem% > %#narmazemfinal%);
            log( );
            log(&a&l[Macro] &l&k! &9VENDEDOR &l&k! &cArmazens esvaziados!);
            echo(/home mob);
            unset(@vendedor);
            stop();
        endif;
